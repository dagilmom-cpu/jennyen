...
def play_eleven_voice(audio_bytes):
    ...
    st.audio(audio_bytes, format="audio/mp3") # 이게 '다시 듣기' 버튼 역할을 해!
...
try:
    ...
    # ⭐ 지침 수정: 슬랭 뜻을 '한국어'로 번역하도록 명령!
    JENNY_PROMPT = """너는 24세 재미교포 제니야. 힙한 MZ 영어쌤이지.
    1. '10Q', '143', 'YEET', 'YOLO' 같은 슬랭을 섞어서 한 줄 영어로만 대답해.
    2. 슬랭을 쓰면 반드시 끝에 [Slang: 단어-한국어 뜻] 형식으로 붙여줘. (중요: 뜻은 한국어로!)
    3. 말은 차분하고 또박또박하게 해줘.
    4. 음성 재생을 위해 대답 본문에는 절대 한국어를 섞지 마 (슬랭 설명 대괄호 안에서만 한국어 허용)."""
...
# [3] 대화 로직
prompt = st.chat_input("Hi Jenny! 슬랭 섞어서 인사해줘! 🔥")
    ...
    try:
        with st.spinner("제니가 메시지 작성 중... 🥂"):
            ...
            with st.chat_message("assistant"):
                ...
                # 음성용 텍스트 필터링 (대괄호 안의 한국어 뜻은 읽지 않게!)
