...
try:
    ...
    st.error("Secrets 설정(Advanced Settings)에서 API 키들을 확인해줘 언니! 😭")
    st.stop()
...
if "user_info" not in st.session_state:
    with st.container():
        ...
        st.subheader("최신 Groq 엔진으로 무장한 제니쌤이랑 밤새 수다 떨자! 🥂")
        ...
        goal = st.text_area("Why English?", placeholder="영어를 배우려는 목적이 뭐야? (예: 발리 서핑 여행, 비즈니스 등)")
...
3. 사용자의 레벨({user['level']})에 맞춰 말하는 속도와 단어 난이도를 조절해줘.
4. 원어민 슬랭 사용 시 본문에서는 **굵게(Bold)** 표시하고, 끝에 [Slang: 단어-한국어 뜻]을 붙여줘. 
...
# [5] 메인 대화 로직
prompt = st.chat_input(f"Hey {user['name']}! What's on your mind? 🥂")
...
if prompt:
    ...
    try:
        with st.spinner("Jenny is catching a wave... 🏄‍♀️"):
            # 최신 모델 'llama-3.3-70b-versatile' 사용!
...
# [6] 사이드바 수업 종료 요약
if st.sidebar.button("🏁 수업 끝! 요약해줘"):
    st.sidebar.success(f"Great sesh today, {user['name']}! ✨")
